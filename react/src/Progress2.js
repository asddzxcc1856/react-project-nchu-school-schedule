...
const Progress2 = () => {
    ...
    const handle_add = (idx) => {
        ...
        for (let i = 0; i < number.length; i++) {
            if (i === 0)
                document.getElementById("thead-td-" + (8 * (parseInt(number[i],16) - 1) + parseInt(numberofclass[0]) + 1)).innerHTML = "<button className='remove' id='" + numberofclass + "'>X</button><br/>" + tr_child[2].innerText + "<br/>(<a href='" + tr_child[1].children[0].href + "'>" + tr_child[1].innerText + "</a>)<br/>" + tr_child[12].innerText + "<br/>" + (tr_child[10].innerText ? tr_child[10].innerText : "");
            else
                document.getElementById("thead-td-" + (8 * (parseInt(number[i],16) - 1) + parseInt(numberofclass[0]) + 1)).innerHTML = tr_child[2].innerText + "<br/>(<a href='" + tr_child[1].children[0].href + "'>" + tr_child[1].innerText + "</a>)<br/>" + tr_child[12].innerText + "<br/>" + (tr_child[10].innerText ? tr_child[10].innerText : "");
        }
        ...
        <React.StrictMode>
            <div>
                <button className="submit" onClick={() => {
                    ...
                    {
                        let text;
                    text = "p_year=" + document.getElementById("sel-year").value + "&v_subject=" +
                        document.getElementById("sel-subject").value + "&v_text=" +
                        ...
                        document.getElementById("sel-mtg").value + "&v_lang=" +
                        document.getElementById("sel-lang").value + "&v_emi=" +
                        document.getElementById("sel-emi").value + "&v_crseno=" +
                        document.getElementById("sel-crseno").value;
                    //console.log(text);
                    //console.log("v_year=&v_career=U&v_excel=0&v_dept=U64F&v_level=&v_lang=&v_text=&v_teach=&v_week=&v_mtg=&v_emi=");
                    fetch('https://nchuclass.axisflow.biz:8080/post2', {
                        method: 'POST',
                        body: text,
                        headers: {
                    ...
                    fetch('https://nchuclass.axisflow.biz:8080/post2', {
                        ...
                        .then(ress => {
                            ...
                            for (let i = 5; i < line.length; i++) {
                                ...
                                line[i].setAttribute("id", "trall-" + i);
                                line[i].children[2].innerText = line[i].children[2].innerText.replace(/[^\u4E00-\u9FA5]/g, '');
                                line[i].children[19].innerHTML = line[i].children[19].innerText ? "<summary></summary><details>" + line[i].children[19].innerText + "</details>" : "";
                            }
                        })
                    }
